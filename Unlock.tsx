...
import { Unlock as UnlockIcon, Loader2, AlertCircle, FileText, ShieldCheck } from 'lucide-react';
...
const Unlock: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await docService.unlock(file, password);
      setResult(res);
      setStatus('completed');
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-20">
      <SEO 
        title="Unlock PDF" 
        description="Remove passwords from PDF files. Decrypt PDFs online."
      />
      
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
            <div className="text-center lg:text-left mb-8">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">Unlock PDF</h1>
                <p className="mt-3 text-lg text-gray-500 dark:text-gray-400">Remove password security from your PDF.</p>
            </div>

            <div className="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 flex items-start gap-3">
                <ShieldCheck className="w-5 h-5 text-green-700 dark:text-green-400 flex-shrink-0 mt-0.5" />
                <div>
                <h4 className="text-sm font-bold text-green-800 dark:text-green-400">Local Decryption</h4>
                <p className="text-sm text-green-700 dark:text-green-300">Your password is used locally to open the file and create an unprotected copy.</p>
                </div>
            </div>
                ...
                <div className="space-y-6 animate-fade-in">
                    ...
                    {status === 'idle' && (
                        <div className="space-y-4 max-w-md mx-auto">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Enter Current Password</label>
                                <input 
                                    type="password" 
                                    value={password}
                            ...
                            >
                                Unlock PDF <UnlockIcon className="ml-2 w-5 h-5" />
                            </button>
                        </div>
                    )}
                    ...
                    {status === 'processing' && (
                        <div className="text-center py-10">
                            ...
                            <p className="text-gray-600 dark:text-gray-300">Unlocking PDF...</p>
                        </div>
                    )}
                    ...
                    {status === 'error' && (
                        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 flex items-start space-x-3">
                            ...
                            <div>
                                <h4 className="text-sm font-medium text-red-800 dark:text-red-400">Unlock Failed</h4>
