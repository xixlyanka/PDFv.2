...
- Все инструменты работают целиком в браузере: конвертация DOCX выполняется через `mammoth` + `html2canvas/jsPDF`, а сжатие PDF использует `pdf.js` и `jsPDF` без обращений к внешним API или backend-роутам.
- Код опирается только на библиотеки, подключаемые в `index.html` (pdf-lib, pdf.js, jsPDF, JSZip, XLSX, mammoth, Tesseract, html2canvas и др.).
- Логирование и серверные endpoints удалены: приложение разворачивается как чистое SPA на Vercel без serverless-функций.
