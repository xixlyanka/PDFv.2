...
- Добавлен серверный эндпоинт `/api/convert-docx`, который конвертирует DOCX → PDF внутри serverless-функции без сторонних API и оставляет клиентский `mammoth` как резервный путь.
- Добавлен серверный эндпоинт `/api/compress`, который сжимает PDF внутри serverless-функции на Vercel; при ошибке фронтенд автоматически использует браузерный компрессор.
...
- Эндпоинты размещены в `api/`, поэтому при деплое на Vercel будут работать как serverless-функции. Фронтенд вызывает `/api/convert-docx` для DOCX → PDF и `/api/compress` для сжатия — никакие внешние ключи не нужны.

## Что нужно для работы бэкенда

- **Переменные среды**: базовые инструменты работают без дополнительных ключей — конвертация DOCX и сжатие PDF выполняются целиком внутри serverless-функций.
- **Лимит загрузки**: serverless-функции принимают файлы до ~25 МБ (см. `api/*.ts` `bodyParser.sizeLimit`). Более крупные файлы нужно предварительно уменьшать или поднимать лимит с учётом ограничений Vercel.
- **Проверка на Vercel**: деплой без дополнительных ключей — эндпоинты работают «из коробки».
...
- Вызовите `GET /api/health` локально или на проде — в ответе будет видно, что встроенные эндпоинты активны и не требуют дополнительных ключей.
