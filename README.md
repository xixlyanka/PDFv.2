...
- Добавлен серверный эндпоинт `/api/compress`, который отправляет PDF в CloudConvert и возвращает сжатый файл; если ключа нет или сервис недоступен, фронтенд сразу использует браузерный компрессор без ошибок для пользователя.
...
- Для сжатия можно указать `CLOUDCONVERT_API_KEY`, чтобы `/api/compress` мог отправлять задания в CloudConvert. Ключ не обязателен: без него сжатие выполняется на клиенте.

## Что нужно для работы бэкенда

- **Переменные среды**: обязательно `DOCX_CONVERT_API_SECRET` (DOCX → PDF). `CLOUDCONVERT_API_KEY` — опционален: с ним сжатие уходит на сервер, без него работает только клиентский компрессор.
...
- **Проверка на Vercel**: внесите нужные ключи в раздел *Project Settings → Environment Variables* и сделайте повторный деплой. Если `CLOUDCONVERT_API_KEY` отсутствует, серверное сжатие отключится, но клиентское продолжит работать.
...
- Вызовите `GET /api/health` локально или на проде — в ответе будет видно, выставлены ли `DOCX_CONVERT_API_SECRET` и `CLOUDCONVERT_API_KEY`. При отсутствии `CLOUDCONVERT_API_KEY` серверное сжатие пропускается, но клиентская компрессия останется доступной.
