...
**Dependencies used on the backend:** `pdf-lib`, `pdfjs-dist` + `@napi-rs/canvas`, `sharp`, `mammoth`, `xlsx`, `tesseract.js`. Все работают в среде Vercel без сторонних сервисов.
...
- `POST /api/convert-docx` — DOCX/XLSX → PDF через `mammoth`, `xlsx` и `pdf-lib` на сервере.
