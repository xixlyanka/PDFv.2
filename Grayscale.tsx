...
import { Monitor, Loader2, AlertCircle, FileText, ShieldCheck } from 'lucide-react';
...
const Grayscale: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await docService.grayscale(file, (msg) => setProgressMsg(msg));
      setResult(res);
      setStatus('completed');
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-20">
      <SEO 
        title="Grayscale PDF" 
        description="Convert colored PDFs to black and white online. Free and secure."
      />
      
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
            <div className="text-center lg:text-left mb-8">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">Grayscale PDF</h1>
                <p className="mt-3 text-lg text-gray-500 dark:text-gray-400">Convert your document to black & white (grayscale) to save ink.</p>
            </div>

            <div className="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 flex items-start gap-3">
                <ShieldCheck className="w-5 h-5 text-green-700 dark:text-green-400 flex-shrink-0 mt-0.5" />
                <div>
                <h4 className="text-sm font-bold text-green-800 dark:text-green-400">Local Conversion</h4>
                <p className="text-sm text-green-700 dark:text-green-300">We render your PDF locally and apply a grayscale filter.</p>
                </div>
            </div>
                ...
                <div className="space-y-6 animate-fade-in">
                    ...
                    {status === 'idle' && (
                        <div className="space-y-4">
                            <p className="text-sm text-gray-600 dark:text-gray-300 text-center">
                                This process will convert every page to an image and remove color. 
                                Text will no longer be selectable, but file size may decrease and printing will be cheaper.
                            </p>
                            <button 
                            ...
                            >
                                Convert to Grayscale <Monitor className="ml-2 w-5 h-5" />
                            </button>
                        </div>
                    )}
                    ...
                    {status === 'processing' && (
                        <div className="text-center py-10">
                            ...
                            <p className="text-gray-600 dark:text-gray-300">Converting...</p>
                    ...
                    {status === 'error' && (
                        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 flex items-start space-x-3">
                            ...
                            <div>
                                <h4 className="text-sm font-medium text-red-800 dark:text-red-400">Failed</h4>
