...
import { Layers, Loader2, AlertCircle, FileText, ShieldCheck } from 'lucide-react';
...
const Flatten: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await docService.flatten(file);
      setResult(res);
      setStatus('completed');
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-20">
      <SEO 
        title="Flatten PDF" 
        description="Merge form fields and annotations into the PDF content online."
      />
      
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
            <div className="text-center lg:text-left mb-8">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">Flatten PDF</h1>
                <p className="mt-3 text-lg text-gray-500 dark:text-gray-400">Make fillable forms and annotations permanent.</p>
            </div>

            <div className="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 flex items-start gap-3">
                <ShieldCheck className="w-5 h-5 text-green-700 dark:text-green-400 flex-shrink-0 mt-0.5" />
                <div>
                <h4 className="text-sm font-bold text-green-800 dark:text-green-400">Secure Flattening</h4>
                <p className="text-sm text-green-700 dark:text-green-300">We process your PDF locally. Forms are merged without leaving your device.</p>
                </div>
            </div>
                ...
                <div className="space-y-6 animate-fade-in">
                    ...
                    {status === 'idle' && (
                        <div className="space-y-4 max-w-md mx-auto">
                            <p className="text-sm text-gray-600 dark:text-gray-300 text-center">
                                This will merge all interactive form fields and annotations into the page content, making them uneditable.
                            </p>
                            ...
                            >
                                Flatten PDF <Layers className="ml-2 w-5 h-5" />
                            </button>
                        </div>
                    )}
                    ...
                    {status === 'processing' && (
                        <div className="text-center py-10">
                            ...
                            <p className="text-gray-600 dark:text-gray-300">Flattening forms...</p>
                        </div>
                    )}
                    ...
                    {status === 'error' && (
                        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 flex items-start space-x-3">
                            ...
                            <div>
                                <h4 className="text-sm font-medium text-red-800 dark:text-red-400">Flatten Failed</h4>
