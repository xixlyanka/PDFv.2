...
export const docxBufferToPdf = async (buffer: Buffer, filename: string): Promise<Buffer> => {
  ...
  return await textToPdfBuffer(text || 'Document contained no readable text.', filename);
};
...
export const xlsxBufferToPdf = async (buffer: Buffer, filename: string): Promise<Buffer> => {
  ...
  return await textToPdfBuffer([`Sheet: ${sheetName}`, '', ...lines].join('\n'), filename);
};
...
export const htmlToPdfBuffer = async (html: string, title = 'html-export'): Promise<Buffer> => {
  const plain = html
    ...
    .replace(/<[^>]+>/g, '')
    .trim();
  return await textToPdfBuffer(plain || 'No HTML content provided.', title);
};
...
const textToPdfBuffer = async (text: string, title: string): Promise<Buffer> => {
  ...
  const lines = wrapText(text.split(/\r?\n/), font, fontSize, maxWidth);
...
const wrapText = (lines: string[], font: any, size: number, maxWidth: number): string[] => {
