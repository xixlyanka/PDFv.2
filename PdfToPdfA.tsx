...
const PdfToPdfA: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await docService.convertToPdfA(file);
      setResult(res);
      setStatus('completed');
      addToast("Converted to PDF/A successfully!", 'success');
    } catch (e: any) {
      setStatus('error');
                    ...
                    {status === 'idle' && (
                        <div className="space-y-4 max-w-sm mx-auto">
                            <p className="text-sm text-gray-600 dark:text-gray-300 text-center">
                                This will embed XMP metadata and attempt to structure the PDF for archival purposes.
                            </p>
                            <button 
                            ...
                            >
                                Convert to PDF/A <Archive className="ml-2 w-5 h-5" />
                            </button>
                        </div>
                    )}
