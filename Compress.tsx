...
const Compress: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-20">
      <SEO 
        title="Compress PDF" 
        description="Reduce PDF file size online. Server processing with client fallback."
      />
         ...
         <div className={showAds ? "lg:col-span-2" : "col-span-1"}>
            <div className="text-center lg:text-left mb-8">
                <div className="flex items-center gap-3 justify-center lg:justify-start mb-3">
                  ...
                  <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200">Client fallback</span>
                </div>
            ...
            <div className="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 flex items-start gap-3">
                ...
                <p className="text-sm text-green-700 dark:text-green-300">This tool uses secure server-side processing. Files are sent to our endpoint, processed immediately, and never stored. If the server fails, compression automatically switches to the in-browser flow.</p>
                </div>
            </div>
                    ...
                    {status === 'idle' && (
                        <div className="bg-white/50 dark:bg-slate-800/50 p-6 rounded-xl border border-gray-100 dark:border-slate-700">
                            ...
                            <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
                                Основное сжатие выполняется на сервере; ползунок задаёт желаемый уровень. При откате в браузер страницы рендерятся в JPEG с понижением качества.
                            </p>
                        </div>
                    )}
